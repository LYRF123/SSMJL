...
class MaskedAutoencoderConvViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
        ...
        self.decoder_embed = nn.Sequential(
            nn.Linear(embed_dim[-1], 100, bias=True),
            nn.GELU(),
            nn.Linear(100, decoder_embed_dim, bias=True)
        )
...
#        torch.nn.init.normal_(self.cls_token, std=.02)
    ...
    def unpatchify(self, x):
        ...
        c = self.in_chans  # number of channels
...
def convmae_convvit_tiny_patch16_dec256d4b(**kwargs):
    model = MaskedAutoencoderConvViT(
        img_size=[224, 56, 28], patch_size=[4, 2, 2], embed_dim=[64, 96, 192], depth=[2, 2, 8], num_heads=3,
...
convmae_convvit_tiny_patch16 = convmae_convvit_tiny_patch16_dec256d4b
